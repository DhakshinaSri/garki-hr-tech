                ...
                <div key={index} className="relative group">
                  {hasSubmenu ? (
                    ...
                    <Link
                      ...
                      className={`text-[20px] font-montserrat font-medium pb-1 transition ${
                        isActive
                          ? "border-b-4 border-[#FA9A2A] text-[#FA9A2A]"
                          : "text-black hover:text-[#d67d10]"
                      }`}
                    >
                      {item.name}
                  ...
                  {hasSubmenu && openDropdown === item.name && (
                    <div className="absolute left-0 top-full mt-3 w-72 bg-white shadow-xl border border-gray-100 rounded-lg py-3 z-40">
                      <h4 className="text-[#FA9A2A] font-semibold text-[16px] px-4 pb-2">
                        {item.name}
                      </h4>
                      <div className="flex flex-col space-y-2 px-4">
                        {item.subMenu.map((sub, i) => (
                          <Link
                            ...
                            className="text-[20px] mt-3 text-gray-800 font-medium hover:text-[#FA9A2A] transition"
                          >
                            {sub.name}
                          </Link>
                        ))}
                ...
                {hasSubmenu ? (
                      ...
                      <div className="flex flex-col mt-6 text-center space-y-6">
                        {item.subMenu.map((sub, i) => (
                          <Link
                            key={i}
                            to={sub.path}
                            className="text-[22px] font-medium text-gray-800 hover:text-[#FA9A2A]"
                            onClick={() => {
                              setIsOpen(false);
                              setOpenDropdown(null);
                            }}
                  ...
                  <Link
                    ...
                    className={`text-[22px] font-montserrat font-medium ${
                      isActive
                        ? "text-[#FA9A2A] border-b-4 border-[#FA9A2A]"
                        : "text-black hover:text-[#d67d10]"
                    }`}
                    onClick={() => setIsOpen(false)}
                  >
